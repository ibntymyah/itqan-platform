...
/**
 * DEMO_MODE: ينشئ مؤسسة ومستخدم مشرف تجريبيين تلقائياً عند التهيئة، فقط
 * لأغراض معاينة الواجهة قبل بناء شاشة onboarding حقيقية. آمن للتشغيل
 * المتكرر (upsert)، لكن يجب حذف هذا القسم بمجرد وجود تسجيل مؤسسات حقيقي —
 * لا تُستخدم بيانات الدخول هذه لأي بيانات طلاب أو مؤسسات فعلية.
 */
const DEMO_ORG_ID = '00000000-0000-0000-0000-000000000001';
const DEMO_EMAIL = 'admin@demo.itqan';
...
async function seedDemoAccount() {
  const org = await prisma.organization.upsert({
    ...
    update: {},
    create: { id: DEMO_ORG_ID, name: 'مؤسسة تجريبية للمعاينة' }
  });

  const role = await prisma.role.upsert({
    where: { orgId_name: { orgId: org.id, name: 'org_admin' } },
    update: {},
    create: { orgId: org.id, name: 'org_admin', isSystem: true }
  });
